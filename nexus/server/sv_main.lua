...
AddEventHandler("onResourceStart", function()
    ...
    if nexus.check_for_updates == true and GetCurrentResourceName() == resourceName then
        local currentVersion = nexus.version -- Get Current Version, dont edit the version!
        ...
        PerformHttpRequest("https://api.github.com/repos/Gerrxt07/nexus/releases/latest", function(responseCode, resultData, resultHeaders) -- Request the latest version from github
            if responseCode == 200 then -- If the request was successfully
                ...
                if currentVersion < latestVersion then -- Check if the current version is lower than the latest version
                    print(Locales[nexus.language]['script_start_but_old']) -- If check is successfully, but the script is old, print the message!
                else
                    print(Locales[nexus.language]['script_start']) -- If all checks are successfully, print the message!
                end
            else
                print(Locales[nexus.language]['script_no_check']) -- If check was not successfully, print the message!
            end
        end, "GET", "", {})
    else
        print(Locales[nexus.language]['script_false_name']) -- If the script name is not the same as the real, intended resource name, print the message!
    end
end)